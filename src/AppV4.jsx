...
function Header() {
  ...
  return <header className="header"><div className="shell header-inner">
    <a href="#start" className="brand" aria-label="Fleissig Startseite"><img src="/logo.png" alt="Fleissig Reinigung" width={54} height={54} /></a>
    ...
    <button className="menu-btn" aria-label={open ? "Menü schliessen" : "Menü öffnen"} aria-expanded={open} onClick={() => setOpen(!open)}>{open ? <X /> : <Menu />}</button>
...
function LegalPage({ type, onClose }) {
  ...
  return <div className="legal-overlay" role="dialog" aria-modal="true" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}><div className="legal-page"><button className="legal-close" onClick={onClose} aria-label="Schliessen"><X /></button><span className="eyebrow">Rechtliches</span><h1>{type === "privacy" ? "Datenschutzerklärung" : "Impressum"}</h1>{type === "privacy" && <p className="legal-intro">Informationen zur Bearbeitung von Personendaten nach dem schweizerischen Datenschutzrecht.</p>}{data.map(([title, text]) => <section key={title}><h2>{title}</h2><p>{text}</p></section>)}</div></div>;
}
...
export default function AppV4() {
  ...
  return <div id="start"><Header /><main>
    <section className="hero"><div className="shell hero-grid"><div><span className="eyebrow">Reinigung im Kanton Aargau</span><h1>Professionelle Reinigung im Aargau.</h1><p className="lead">Faire Stundenpreise ab CHF 55. Schnelle Antwort per WhatsApp oder Telefon — wir sprechen Deutsch.</p><div className="hero-actions"><WhatsAppButton label="Reinigungstermin anfragen" /><a className="btn btn-secondary" href={`tel:${CONFIG.phone.replace(/\s/g, "")}`} onClick={() => trackLead("phone")}><Phone size={19} /> Anrufen</a></div><small><Clock3 size={15} /> Antwort in der Regel innerhalb von 15 Minuten · werktags 8–18 Uhr</small></div><img className="hero-img" src="/hero-clean-bathroom.webp" alt="Sauber gereinigtes Badezimmer" width={600} height={800} fetchPriority="high" /></div></section>
    <TrustStrip />
    <section className="section results" id="ergebnisse"><div className="shell"><span className="eyebrow">Vorher / Nachher</span><h2>Ergebnisse, die man sieht.</h2><div className="result-grid"><figure><img src="/vorher-nachher-duschkabine.jpg" alt="Duschkabine vor und nach der Reinigung" width={1280} height={836} loading="lazy" decoding="async" /><figcaption>Duschkabine — Kalk und Wasserflecken entfernt</figcaption></figure><figure><img src="/vorher-nachher-kochfeld.jpg" alt="Kochfeld vor und nach der Reinigung" width={1280} height={836} loading="lazy" decoding="async" /><figcaption>Kochfeld — eingebrannte Rückstände entfernt</figcaption></figure></div></div></section>
    <Services />
  ...
  </main><footer><div className="shell footer"><div><strong>Fleissig</strong><span>Reinigung & Gartenpflege im Kanton Aargau</span><span>Mo–Fr 8–18 Uhr · {CONFIG.location}</span></div><div className="footer-links"><button onClick={() => setLegal("imprint")}>Impressum</button><button onClick={() => setLegal("privacy")}>Datenschutz</button><a href={`mailto:${CONFIG.email}`}>E-Mail</a></div></div></footer>{legal && <LegalPage type={legal} onClose={() => setLegal(null)} />}<CookieBanner /><a className="floating-wa" href={waLink("regular")} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp öffnen"><MessageCircle size={25} /></a><style>{styles}</style></div>;
}

const styles = `
:root{--green:#3d7b4f;--dark:#275c39;--pale:#f1f7f2;--ink:#171917;--muted:#5f6963;--line:#e2e8e3;--wa:#25d366}*{box-sizing:border-box}html{scroll-behavior:smooth}section[id]{scroll-margin-top:88px}:focus-visible{outline:3px solid var(--green);outline-offset:2px}.contact :focus-visible,footer :focus-visible{outline-color:#fff}body{margin:0;font-family:'Plus Jakarta Sans',sans-serif;color:var(--ink)}.shell{width:min(1120px,calc(100% - 40px));margin:auto}.header{position:sticky;top:0;z-index:50;background:rgba(255,255,255,.97);border-bottom:1px solid var(--line);backdrop-filter:blur(10px)}.header-inner{height:72px;display:flex;align-items:center;gap:28px}.brand img{width:54px;height:54px;object-fit:contain}.desktop-nav{display:flex;gap:24px;margin-left:auto}.desktop-nav a{color:#404842;text-decoration:none;font-size:14px;font-weight:600;transition:color .15s}.desktop-nav a:hover{color:var(--green)}.header-cta{display:flex;align-items:center;gap:7px;background:var(--green);color:white;text-decoration:none;padding:10px 15px;border-radius:10px;font-weight:700;font-size:14px;transition:background-color .15s}.header-cta:hover{background:var(--dark)}.menu-btn{display:none;margin-left:auto;border:1px solid var(--line);background:white;border-radius:10px;padding:8px;cursor:pointer}.mobile-nav{border-top:1px solid var(--line);background:white;padding:12px 0 20px}.mobile-nav .shell{display:flex;flex-direction:column;gap:8px}.mobile-nav a:not(.btn){padding:11px 2px;text-decoration:none;color:var(--ink)}.hero{background:linear-gradient(145deg,#edf7ef,#fff 68%);padding:72px 0}.hero-grid{display:grid;grid-template-columns:1.05fr .95fr;gap:64px;align-items:center}.eyebrow{display:block;color:var(--green);font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:.1em;margin-bottom:14px}.hero h1{font-size:clamp(42px,6vw,68px);line-height:1.05;letter-spacing:-.045em;margin:0 0 20px}.lead{font-size:18px;line-height:1.65;color:var(--muted);max-width:590px;margin:0 0 26px}.hero-actions{display:flex;gap:12px;flex-wrap:wrap}.hero small{display:flex;gap:7px;align-items:center;color:#5b665f;margin-top:17px}.hero-img{width:100%;height:520px;object-fit:cover;border-radius:22px;box-shadow:0 18px 48px rgba(34,82,48,.16)}.btn{display:inline-flex;align-items:center;justify-content:center;gap:9px;min-height:50px;padding:12px 20px;border-radius:11px;text-decoration:none;font-size:15px;font-weight:700;border:2px solid transparent;transition:background-color .15s,border-color .15s,color .15s,transform .15s,box-shadow .15s}.btn:active{transform:scale(.98)}.btn-wa{background:var(--wa);color:white}.btn-wa:hover{background:#1cbd58;box-shadow:0 6px 18px rgba(37,211,102,.32)}.btn-secondary{background:white;color:var(--green);border-color:var(--green)}.btn-secondary:hover{background:var(--pale)}.btn-call{background:white;color:var(--dark);margin-top:10px}.btn-call:hover{background:var(--pale)}.trust{border-top:1px solid var(--line);border-bottom:1px solid var(--line)}.trust-grid{display:grid;grid-template-columns:repeat(4,1fr);padding:22px 0}.trust-grid>div{display:flex;align-items:center;justify-content:center;gap:9px;font-size:13px;font-weight:650;color:#465048;border-right:1px solid var(--line)}.trust-grid>div:last-child{border:0}.trust svg{color:var(--green)}.section{padding:78px 0}.results{background:white}.result-grid{display:grid;grid-template-columns:1fr 1fr;gap:20px}.result-grid figure{margin:0}.result-grid img{width:100%;height:auto;border-radius:18px;box-shadow:0 8px 28px rgba(0,0,0,.09)}.result-grid figcaption{margin-top:11px;font-size:14px;font-weight:600;color:var(--muted)}.services{background:var(--pale)}.services h2,.results h2,.contact h2{font-size:clamp(31px,4vw,48px);line-height:1.12;letter-spacing:-.035em;margin:0 0 34px}.cards{display:grid;grid-template-columns:repeat(3,1fr);gap:18px}.card{background:white;border:1px solid #dfe8e1;border-radius:18px;padding:25px;display:flex;flex-direction:column;transition:box-shadow .18s,transform .18s}.card:hover{transform:translateY(-3px);box-shadow:0 14px 34px rgba(34,82,48,.1)}.icon{width:48px;height:48px;border-radius:13px;background:#e9f4ec;color:var(--green);display:grid;place-items:center;margin-bottom:20px}.card h3{font-size:21px;margin:0 0 10px}.card strong{color:var(--green);font-size:19px}.card p{color:var(--muted);line-height:1.55;margin:12px 0 22px}.card .btn{margin-top:auto}.review{padding:62px 0;text-align:center;background:#fafcfa}.stars{display:flex;justify-content:center;color:#e5a33f;gap:3px}.review blockquote{font-size:clamp(20px,3vw,29px);font-weight:600;line-height:1.45;max-width:800px;margin:18px auto}.review strong{color:var(--green)}.contact{background:var(--dark);color:white}.contact-grid{display:grid;grid-template-columns:1fr .7fr;gap:60px;align-items:center}.contact p{color:#d3e3d7;line-height:1.65}.contact-grid>div:last-child{display:flex;flex-direction:column}.light{color:#bfe4c9}footer{background:#17251c;color:white;padding:28px 0}.footer{display:flex;justify-content:space-between;gap:30px}.footer>div{display:flex;flex-direction:column;gap:5px}.footer span{color:#aebbb2;font-size:12px}.footer-links{align-items:flex-end}.footer-links button,.footer-links a{background:none;border:0;color:#c3cec6;font:inherit;font-size:12px;text-decoration:none;cursor:pointer;padding:2px;transition:color .15s}.footer-links button:hover,.footer-links a:hover{color:white}.floating-wa{display:none;position:fixed;right:18px;bottom:18px;width:58px;height:58px;border-radius:50%;background:var(--wa);color:white;place-items:center;box-shadow:0 8px 24px rgba(0,0,0,.22);z-index:45;transition:transform .15s,box-shadow .15s}.floating-wa:hover{transform:scale(1.07)}.legal-overlay{position:fixed;inset:0;background:rgba(0,0,0,.58);z-index:100;overflow:auto;padding:30px}.legal-page{position:relative;max-width:800px;margin:auto;background:white;border-radius:18px;padding:42px}.legal-close{position:absolute;right:18px;top:18px;background:white;border:1px solid var(--line);border-radius:10px;padding:7px;cursor:pointer;transition:background-color .15s}.legal-close:hover{background:var(--pale)}.legal-page h1{font-size:40px;margin:0 0 14px}.legal-page section{padding:18px 0;border-top:1px solid var(--line)}.legal-page section h2{font-size:18px;margin:0 0 8px}.legal-page p{color:var(--muted);line-height:1.65;margin:0}.legal-intro{margin-bottom:26px!important}.cookie{position:fixed;left:20px;right:20px;bottom:20px;z-index:90;max-width:920px;margin:auto;background:white;border:1px solid var(--line);box-shadow:0 14px 45px rgba(0,0,0,.18);border-radius:16px;padding:18px;display:flex;align-items:center;justify-content:space-between;gap:24px}.cookie p{margin:5px 0 0;color:var(--muted);font-size:13px}.cookie-actions{display:flex;gap:9px}.cookie button{border:1px solid var(--green);background:white;color:var(--green);border-radius:9px;padding:10px 13px;font-weight:700;cursor:pointer;transition:background-color .15s,color .15s}.cookie button:hover{background:var(--pale)}.cookie .accept{background:var(--green);color:white}.cookie .accept:hover{background:var(--dark)}
